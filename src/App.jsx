...
// Written to sessionStorage (not localStorage) while a focus session is
// running. sessionStorage survives page refreshes and in-app navigation
// within the same tab, but is cleared the moment the tab/app is actually
// closed — which is exactly the signal we want to tell "switched tabs" (fine,
// keep growing) apart from "closed the app" (wither the plant).
const GROVE_ACTIVE_TOKEN_KEY = "grove.active.token";
...
function focusDefaultState() {
  ...
  return {
    ...
    sessions: [],
    achievements: [],
    // A session that's currently in progress, persisted here (not just in
    // component state) so it survives switching views/tabs. See
    // groveResolveStaleSession below for how this gets finalized if the
    // app/tab is actually closed mid-session.
    activeSession: null,
    settings: {
      masterVolume: 0.6,
      ...
      warnOnLeave: true,
      theme: "light",
      // Forest-app-style "Time Exceeded": once a session hits its planned
      // duration, keep counting instead of stopping. Toggleable by the
      // user above the timer circle on the Focus page (see GroveFocusPage).
      trackOvertime: true,
    },
  };
}
...
const focusStore = {
  ...
  setActiveSession(active) {
    focusStore.set((s) => ({ ...s, activeSession: active }));
  },
  clearActiveSession() {
    focusStore.set((s) => ({ ...s, activeSession: null }));
  },
  resetAll() {
    focusCache = focusDefaultState();
...
// If a focus session was left running when the tab/app was actually closed
// (as opposed to just switched away from or navigated within the site), the
// sessionStorage token from that session won't exist anymore in this fresh
// tab. In that case we finalize it here as a withered/broken session —
// crediting whatever minutes it had already accrued — instead of leaving it
// stuck as "active" forever. Runs once, as soon as the app script loads.
function groveResolveStaleSession() {
  if (typeof window === "undefined") return;
  const s = focusLoad();
  const active = s.activeSession;
  if (!active) return;
  const sameTab = window.sessionStorage.getItem(GROVE_ACTIVE_TOKEN_KEY) === active.token;
  if (sameTab) return; // this tab is still the one running it — leave it alone
  const rawElapsedMin = Math.max(0, Math.round((Date.now() - active.startedAt) / 60000));
  const elapsedMin = active.overtimeEnabled ? rawElapsedMin : Math.min(active.duration, rawElapsedMin);
  const exceededMin = Math.max(0, elapsedMin - active.duration);
  const next = { ...s, activeSession: null };
  if (elapsedMin > 0) {
    const session = {
      id: uid("s"),
      ...
      plannedMin: active.duration,
      actualMin: elapsedMin,
      exceededMin,
      overtime: exceededMin > 0,
      plantId: active.plantId,
      taskId: active.taskId,
      status: "broken",
      reward: groveComputeReward(elapsedMin, "broken"),
    };
    next.sessions = [session, ...s.sessions];
    next.coins = (s.coins || 0) + session.reward;
    const newlyUnlocked = ACHIEVEMENTS.filter(
      (a) => a.progress(next) >= a.goal && !next.achievements.some((x) => x.id === a.id)
    ).map((a) => ({ id: a.id, unlockedAt: Date.now() }));
    if (newlyUnlocked.length) next.achievements = [...next.achievements, ...newlyUnlocked];
  }
  focusPersist(next);
}
groveResolveStaleSession();
...
// The dial is a complete circle — min (5) and max (120) both sit at the
// top, 12 o'clock. To give it a hard stop instead of wrapping, we don't
// map the pointer's absolute angle to a value on every move (that's what
// caused the wrap: crossing the top instantly jumped from 120 to 5).
// Instead, once a drag starts we track how far the pointer has rotated
// since the previous move and add that delta to the current value,
// clamping at min/max. So dragging clockwise stops dead at 120, and the
// only way back down is to reverse the drag direction — just like a real
// dial with an end stop.
const TIMER_WHEEL_GAP_DEG = 0;
const TIMER_WHEEL_SWEEP = 360 - TIMER_WHEEL_GAP_DEG;
const TIMER_WHEEL_START_ANGLE = -90 + TIMER_WHEEL_GAP_DEG / 2;
...
const TIMER_WHEEL_DEFAULT_SNAP_POINTS = [5, 10, 15, 25, 30, 45, 60, 90, 120];
...
function TimerWheel({
  ...
  // Holds the pointer angle + raw (unsnapped) value from the last move,
  // so each new move can be measured as a delta from it rather than as
  // an absolute angle-to-value mapping.
  const dragRef = useRef({ angle: 0, raw: value });

  const angleFromClientPoint = useCallback((clientX, clientY) => {
  ...
  // Absolute angle -> value. Used only once, on pointer-down, so a
  // click/tap anywhere on the ring jumps straight to that position.
  const valueFromAngle = useCallback(
    (angleDeg) => {
      let rel = angleDeg - TIMER_WHEEL_START_ANGLE;
      rel = ((rel % 360) + 360) % 360;
  ...
  const handlePointerMove = useCallback(
    (e) => {
      ...
      const { angle: lastAngle, raw: lastRaw } = dragRef.current;
      // Shortest signed rotation since the last move (range -180..180),
      // so continuing to drag past a boundary can't wrap to the other side.
      let delta = angle - lastAngle;
      delta = (((delta + 180) % 360) + 360) % 360 - 180;
      const deltaValue = (delta / 360) * (max - min);
      const nextRaw = Math.min(max, Math.max(min, lastRaw + deltaValue));
      dragRef.current = { angle, raw: nextRaw };
      onChange(applySnap(nextRaw));
    },
    [onChange, applySnap, min, max, angleFromClientPoint]
  );
  ...
  const startDragging = (e) => {
    ...
    setDragging(true);
    const angle = angleFromClientPoint(e.clientX, e.clientY);
    const raw = valueFromAngle(angle);
    dragRef.current = { angle, raw };
    onChange(applySnap(raw));
  ...
  const sweepFrac = TIMER_WHEEL_SWEEP / 360;
  ...
  return (
    ...
    >
      <svg width={size} height={size} className="absolute inset-0" style={{ transform: `rotate(${TIMER_WHEEL_START_ANGLE}deg)` }}>
        <circle
          cx={cx}
          cy={cy}
          r={r}
          ...
          strokeWidth={10}
          strokeDasharray={`${trackDash * sweepFrac} ${trackDash}`}
        />
        <circle
          cx={cx}
          cy={cy}
          ...
          strokeWidth={10}
          strokeLinecap="round"
          strokeDasharray={`${trackDash * sweepFrac * activeFrac} ${trackDash}`}
        />
        {!disabled && (
          <circle
            cx={cx}
...
/* ---------------------------------------------------------
   ROOT APP — local persistence
   Everything the app previously kept only in React state
   (lists, tasks, habits, contracts, reflection, notifications)
   is now mirrored to localStorage so it survives a refresh or
   closing the tab. This is per-browser/per-device (not synced
   to an account), which matches what was asked for.
--------------------------------------------------------- */
const APP_STORE_KEY = "loggd.app.v1";
...
export default function App() {
  ...
  // Load once on mount. Kept outside any single useState initializer so
  // every field below can pull from the same saved snapshot.
  const savedState = useMemo(() => loadAppState(), []);

  /* ---------- Tasks + Lists (shared across Home / Tasks / Focus Session) ---------- */
  const [lists, setLists] = useState(savedState?.lists ?? [{ id: "tasks", name: "Tasks", emoji: "🏠" }]);
  ...
  const [trackedHabits, setTrackedHabits] = useState(
    savedState?.trackedHabits ?? [{ id: uid("hab"), catalogId: "cat-1", name: "Run daily", icon: "Flame", log: {} }]
  );
  ...
  const [scorecardHabits, setScorecardHabits] = useState(savedState?.scorecardHabits ?? []);

  // Habit contracts
  const [contracts, setContracts] = useState(
    savedState?.contracts ?? [{ id: uid("con"), name: "No sugar for 21 days", active: true, log: {}, createdAt: Date.now() }]
  );

  // Reflection
  const [reflection, setReflection] = useState(savedState?.reflection ?? { text: "", mood: null });
  ...
  // Mirror everything above to localStorage whenever any of it changes.
  useEffect(() => {
    saveAppState({ lists, tasks, trackedHabits, scorecardHabits, contracts, reflection, notifications });
  ...
  return (
    <div className="flex min-h-screen w-full flex-col overflow-x-hidden bg-slate-950 text-slate-200 md:flex-row" style={{ fontFamily: "Inter, ui-sans-serif, system-ui" }}>
      <style>{`
        ...
        .focus-grove-theme {
          ...
          --broken: #f87171;
          --overtime: #f5a524;
          --overtime-soft: rgba(245, 165, 36, 0.14);
          --background: #020617;
          --foreground: #f1f5f9;
          --card: #0f172a;
      ...
      `}</style>
      {/* Persistent sidebar — only shown from the md breakpoint up. On
          mobile it used to render at a fixed w-56 regardless of viewport,
          which is the "vertical line" that ate a big chunk of a phone
          screen and squeezed everything else down. */}
      <Sidebar ctx={ctx} />

      {/* Mobile-only top bar + slide-in drawer, replacing the fixed sidebar
          under the md breakpoint so the full viewport width is available
          for page content. */}
      <MobileTopBar ctx={ctx} onOpenMenu={() => setMobileNavOpen(true)} />
      {mobileNavOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          <div className="absolute inset-0 bg-black/60" onClick={() => setMobileNavOpen(false)} />
          <div className="relative z-10 h-full">
            <Sidebar ctx={ctx} mobile onClose={() => setMobileNavOpen(false)} />
          </div>
        </div>
      )}
...
function Sidebar({ ctx, mobile = false, onClose }) {
  const { view, setView, unreadCount } = ctx;

  // On mobile this same nav is rendered inside a slide-in drawer, so
  // picking an item should also close the drawer.
  function go(v) {
    setView(v);
    if (onClose) onClose();
  }
  ...
  return (
    <aside
      className={
        mobile
          ? "flex h-full w-72 max-w-[82vw] flex-shrink-0 flex-col overflow-y-auto bg-slate-950 px-3 py-4 shadow-xl"
          : "hidden w-56 flex-shrink-0 flex-col border-r border-slate-800 bg-slate-950 px-3 py-4 md:flex"
      }
    >
      <div className="mb-6 flex items-center justify-between gap-2 px-2">
        ...
        {mobile && (
          <button onClick={onClose} className="rounded-md p-1 text-slate-400 hover:bg-slate-900 hover:text-slate-200" title="Close menu">
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col gap-0.5">
        <NavItem icon={HomeIcon} label="Home" active={view === "home"} onClick={() => go("home")} />
        <NavItem icon={Bell} label="Notifications" active={view === "notifications"} onClick={() => go("notifications")} badge={unreadCount || null} />
      </div>

      <div className="mb-1 mt-5 px-2 text-[11px] font-semibold tracking-wider text-slate-600">TRACK</div>
      <div className="flex flex-col gap-0.5">
        <NavItem icon={RefreshCw} label="Habits" active={view === "habits"} onClick={() => go("habits")} />
        <NavItem icon={ClipboardList} label="Tasks" active={view === "tasks"} onClick={() => go("tasks")} />
        <NavItem icon={Clock} label="Focus Session" active={view === "focus"} onClick={() => go("focus")} />
        <NavItem icon={ListChecks} label="Habit Scorecard" active={view === "habitScorecard"} onClick={() => go("habitScorecard")} />
      ...
      <div className="flex flex-col gap-0.5">
        <NavItem icon={Target} label="Goals" active={view === "goals"} onClick={() => go("goals")} />
...
// Compact top bar shown only below the md breakpoint, in place of the
// always-visible sidebar. Opens the same nav as a slide-in drawer so
// content below gets the full viewport width instead of being squeezed
// next to a fixed-width column.
function MobileTopBar({ ctx, onOpenMenu }) {
  const { unreadCount } = ctx;
  return (
    <div className="flex flex-shrink-0 items-center justify-between border-b border-slate-800 bg-slate-950 px-3 py-2.5 md:hidden">
      <div className="flex items-center gap-2">
        <div className="flex h-6 w-6 items-center justify-center rounded-md bg-emerald-500">
          <div className="h-2.5 w-2.5 rounded-sm bg-slate-950" />
        </div>
      ...
      <button onClick={onOpenMenu} className="relative rounded-md p-1.5 text-slate-300 hover:bg-slate-900" title="Open menu">
        <Menu size={20} />
        {unreadCount ? (
          <span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-red-500 px-0.5 text-[9px] font-semibold text-white">
            {unreadCount}
          </span>
        ) : null}
      </button>
...
function GroveSwitch({ checked, onCheckedChange, disabled = false }) {
  return (
    <button
      type="button"
      role="switch"
...
// Formats overtime as "+HH:MM:SS" (Forest-app style) once a session has
// run past its planned duration; hours are only shown once they're needed.
function groveFormatExceeded(totalSeconds) {
...
function groveFormatTodayTotal(totalSeconds) {
  const totalMin = Math.floor(totalSeconds / 60);
  if (totalMin <= 0) return "No focus time yet today — start your first session.";
  const h = Math.floor(totalMin / 60);
  const m = totalMin % 60;
  if (h > 0) return `${h}h ${m}m focused today`;
  return `${m}m focused today`;
}
...
function GroveFocusPage({ ctx }) {
  ...
  // If a session is already running (persisted in the focus store under
  // the same browser tab — see GROVE_ACTIVE_TOKEN_KEY), rehydrate from it
  // so switching to another view/page and coming back to Focus Session
  // resumes right where it was instead of resetting.
  const initialActive = useMemo(() => {
    if (typeof window === "undefined") return null;
    const active = focusStore.get().activeSession;
    if (active && window.sessionStorage.getItem(GROVE_ACTIVE_TOKEN_KEY) === active.token) {
      return active;
    }
    return null;
  }, []);

  const [duration, setDuration] = useState(initialActive?.duration ?? 25);
  const [plantId, setPlantId] = useState(initialActive?.plantId ?? equippedPlant);
  const [taskId, setTaskId] = useState(initialActive?.taskId ?? null);
  // Whether "Time Exceeded" tracking is on for this session — editable only
  // while idle (see the toggle above the timer circle), and carried along
  // with the persisted active session so a tab switch/reload mid-overtime
  // rehydrates correctly instead of resetting the choice.
  const [trackOvertime, setTrackOvertime] = useState(initialActive?.overtimeEnabled ?? settings.trackOvertime ?? true);
  const initialElapsedSec = initialActive ? (Date.now() - initialActive.startedAt) / 1000 : 0;
  const initialTotalSec = (initialActive?.duration ?? 25) * 60;
  const initialStatus = !initialActive
    ? "idle"
    : initialElapsedSec >= initialTotalSec && (initialActive.overtimeEnabled ?? settings.trackOvertime)
    ? "overtime"
    : "running";
  ...
  const startedAtRef = useRef(initialActive ? initialActive.startedAt : null);
  ...
  // How far past the planned duration the session has run. Only nonzero
  // once status is "overtime" — this drives the separate "Time Exceeded"
  // readout and gets saved on the session when it's finally ended.
  const exceededSec = status === "overtime" ? Math.max(0, elapsedSec - totalSec) : 0;

  useEffect(() => {
    if (status !== "running" || elapsedSec < totalSec) return;
    if (trackOvertime) {
      // Don't stop the timer — hand off to the "Time Exceeded" counter
      // instead of finishing the session, Forest-app style.
      setStatus("overtime");
    } else {
      finishSession("success");
    }
  }, [elapsedSec, status, totalSec, trackOvertime]); // eslint-disable-line react-hooks/exhaustive-deps

  // Switching browser tabs, or navigating anywhere else within the site,
  // no longer breaks or penalizes a running session — the timer is driven
  // by wall-clock time (startedAtRef), not by this tab staying focused. A
  // session only withers if the app/tab is actually closed mid-session
  // (handled by groveResolveStaleSession on next load, via
  // GROVE_ACTIVE_TOKEN_KEY). This just offers an optional, non-punishing
  // reminder toast for people who opted into "warn on leave".
  useEffect(() => {
    if (status !== "running" || !settings.warnOnLeave) return;
    const onVis = () => {
      if (document.visibilityState === "hidden") {
        toast.warning("Your session keeps running in the background — come back anytime.");
      }
    };
  ...
  function startSession() {
    ...
    const token = uid("tok");
    if (typeof window !== "undefined") {
      window.sessionStorage.setItem(GROVE_ACTIVE_TOKEN_KEY, token);
    }
    startedAtRef.current = startedAt;
    setElapsedSec(0);
    setStatus("running");
    focusStore.setActiveSession({ token, startedAt, duration, plantId, taskId, overtimeEnabled: trackOvertime });
  }

  function finishSession(finalStatus) {
    if (!startedAtRef.current) return;
    const wasOvertime = status === "overtime";
    const plannedMin = duration;
    // Exceeded time is only ever credited once the session actually
    // entered overtime — a "give up" before reaching the planned duration
    // stays capped at the elapsed minutes, same as before this feature.
    const exceededMin = wasOvertime ? Math.max(0, Math.round((elapsedSec - totalSec) / 60)) : 0;
    const actualMin = wasOvertime ? plannedMin + exceededMin : Math.min(duration, Math.round(elapsedSec / 60));
    const finalized = finalStatus;
    ...
    const session = {
      ...
      // Time Exceeded feature: planned vs. actual vs. how much ran over.
      plannedMin,
      actualMin,
      exceededMin,
    ...
    if (typeof window !== "undefined") window.sessionStorage.removeItem(GROVE_ACTIVE_TOKEN_KEY);
    focusStore.clearActiveSession();
    if (finalized === "success" && taskId) {
      toggleTaskComplete(taskId);
    }
    setStatus("finished");
    setReward({ coins: coinsEarned, withered: finalized !== "success", exceededMin });
  ...
  // Manually ends a session that's currently in overtime — the planned
  // duration was already completed, so this always finishes as a success,
  // crediting the extra time spent focusing.
  function endOvertimeSession() {
    finishSession("success");
  }
  ...
  const progress = status === "running" ? elapsedSec / totalSec : status === "overtime" || status === "finished" ? 1 : 0;
  const unlockedPlants = useMemo(() => plants.filter((p) => p.unlocked), [plants]);

  // Total time focused today, shown regardless of session status — even if
  // the plant withered from the app being closed mid-session, the minutes
  // already earned still count and stay visible.
  const allSessions = useFocusStore((s) => s.sessions);
  const todayFocusSeconds = useMemo(() => {
    const todayKey = dayKey(Date.now());
    const finishedTodaySec = allSessions
      .filter((s) => dayKey(s.startedAt) === todayKey)
      .reduce((sum, s) => sum + s.actualMin * 60, 0);
    const liveSec = status === "running" || status === "overtime" ? Math.max(0, elapsedSec) : 0;
    return finishedTodaySec + liveSec;
  }, [allSessions, status, elapsedSec]);
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="flex flex-col items-center">
        <label
          className="mb-4 flex items-center gap-2.5 rounded-full py-1.5 pl-3.5 pr-2"
          style={{ border: "1px solid var(--border)", background: "var(--card)", opacity: status !== "idle" ? 0.7 : 1 }}
        >
          ...
          <GroveSwitch
            ...
            onCheckedChange={(v) => {
              setTrackOvertime(v);
              focusStore.updateSettings({ trackOvertime: v });
            }}
          />
        </label>
        ...
        <div className="relative">
          ...
          >
            <div className="flex flex-col items-center gap-1">
              <PlantSvg id={plantId} progress={status === "idle" ? 0.15 : Math.max(0.1, progress)} size={150} />
              ...
              {status === "overtime" && (
                <div
                  ...
                  style={{ color: "var(--overtime)" }}
                  title="Time spent focusing beyond your planned duration"
                >
                  {groveFormatExceeded(exceededSec)} exceeded
                </div>
              )}
            </div>
          </TimerWheel>
        ...
        <p className="mt-3 text-sm" style={{ color: "var(--muted-foreground)" }}>
          {groveFormatTodayTotal(todayFocusSeconds)}
        </p>
            ...
            <GroveCardContent className="flex items-center justify-between gap-3 py-6">
              <div>
                ...
                <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
                  {reward.withered
                    ? "Partial credit — every minute still counts."
                    : reward.exceededMin > 0
                    ? `Great focus. You went ${reward.exceededMin}m past your plan.`
                    : "Great focus. Your plant grew fully."}
                </p>
              </div>
        ...
        <div className="mt-6 flex gap-3">
          ...
          {status === "overtime" && (
            <GroveButton size="lg" onClick={endOvertimeSession} className="gap-2">
              <CheckCircle2 className="h-4 w-4" /> End session
            </GroveButton>
          )}
          {status === "finished" && (
          ...
          {selectedSessions.map((s) => (
            <div key={s.id} className="flex items-center justify-between rounded-lg px-4 py-2.5 text-sm" style={{ border: "1px solid var(--border)", background: "var(--card)" }}>
              <span className="flex items-center gap-1.5">
                ...
                {s.exceededMin > 0 && (
                  <span className="font-medium" style={{ color: "var(--overtime)" }} title="Time spent beyond the planned duration">
                    (+{s.exceededMin}m exceeded)
                  </span>
                )}
              </span>
              <span
...
/* ---- pages/forest.tsx (ported) ----
   Expanded from a plain "sessions grouped by day" list into a full
   Day/Week/Month/Year overview (distribution, weekday/hour patterns,
   trend vs previous periods, tag breakdown) plus a forest bed that
   grows one real plant per finished session, similar in spirit to
   the Forest app's own Overview screen. ---- */
function groveDayLabel(ts) {
  return new Date(ts).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" });
}

const GROVE_PERIOD_TABS = [
  { id: "day", label: "Day" },
  { id: "week", label: "Week" },
...
function groveModeNoun(mode) {
  return { day: "Day", week: "Week", month: "Month", year: "Year" }[mode];
}

function groveStartOfDay(d) {
  const x = new Date(d);
  x.setHours(0, 0, 0, 0);
  return x;
}
function groveStartOfWeek(d) {
  const x = groveStartOfDay(d);
  x.setDate(x.getDate() - x.getDay());
  return x;
}
function groveStartOfMonth(d) {
  return new Date(d.getFullYear(), d.getMonth(), 1);
}
function groveStartOfYear(d) {
  return new Date(d.getFullYear(), 0, 1);
}

function grovePeriodRange(anchor, mode) {
  if (mode === "day") {
    const start = groveStartOfDay(anchor);
    const end = new Date(start);
    end.setDate(end.getDate() + 1);
    return { start, end };
  }
  if (mode === "week") {
    const start = groveStartOfWeek(anchor);
    const end = new Date(start);
    end.setDate(end.getDate() + 7);
    return { start, end };
  }
  if (mode === "month") {
    const start = groveStartOfMonth(anchor);
    const end = new Date(start.getFullYear(), start.getMonth() + 1, 1);
    return { start, end };
  }
  const start = groveStartOfYear(anchor);
  const end = new Date(start.getFullYear() + 1, 0, 1);
  return { start, end };
}

function groveShiftPeriod(anchor, mode, dir) {
...
function grovePeriodLabel(anchor, mode) {
  if (mode === "day") return anchor.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" });
  if (mode === "week") {
    const { start, end } = grovePeriodRange(anchor, "week");
...
// Buckets the given sessions into the x-axis of the "Focused Time
// Distribution" chart: hours for Day, weekdays for Week, calendar days
// for Month, months for Year.
function groveBucketsForPeriod(anchor, mode, sessions) {
  const { start } = grovePeriodRange(anchor, mode);
  const raw = [];
  if (mode === "day") {
    for (let h = 0; h < 24; h++) {
      ...
      raw.push({ label: `${h}`, start: bStart, end: bEnd });
    }
  } else if (mode === "week") {
    for (let i = 0; i < 7; i++) {
      const bStart = new Date(start);
      ...
      raw.push({ label: bStart.toLocaleDateString(undefined, { weekday: "short" }), start: bStart, end: bEnd });
    }
  } else if (mode === "month") {
    const daysInMonth = new Date(start.getFullYear(), start.getMonth() + 1, 0).getDate();
    for (let d = 1; d <= daysInMonth; d++) {
      const bStart = new Date(start.getFullYear(), start.getMonth(), d);
      const bEnd = new Date(start.getFullYear(), start.getMonth(), d + 1);
      raw.push({ label: `${bStart.getMonth() + 1}/${d}`, start: bStart, end: bEnd });
    }
  } else {
    for (let m = 0; m < 12; m++) {
      const bStart = new Date(start.getFullYear(), m, 1);
      const bEnd = new Date(start.getFullYear(), m + 1, 1);
      raw.push({ label: `${m + 1}`, start: bStart, end: bEnd });
    }
  }
  return raw.map((b) => ({
    label: b.label,
    value: sessions.reduce((sum, s) => {
      if (s.status !== "success") return sum;
      return s.startedAt >= b.start.getTime() && s.startedAt < b.end.getTime() ? sum + s.actualMin : sum;
    }, 0),
  }));
}

// Average focused minutes per weekday, across every week on record —
// "in general", independent of the period currently selected above.
function groveWeekdayPattern(sessions) {
  const totals = Array(7).fill(0);
  const weeksSeen = Array.from({ length: 7 }, () => new Set());
  for (const s of sessions) {
    if (s.status !== "success") continue;
    const d = new Date(s.startedAt);
    const wd = d.getDay();
    totals[wd] += s.actualMin;
    weeksSeen[wd].add(groveKeyFor(groveStartOfWeek(d)));
  }
  const labels = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
  return labels.map((label, i) => ({
    label,
    value: weeksSeen[i].size ? Math.round(totals[i] / weeksSeen[i].size) : 0,
  }));
}

// Average focused minutes per hour of day, across every day on record.
function groveHourPattern(sessions) {
  const totals = Array(24).fill(0);
  const daysSeen = Array.from({ length: 24 }, () => new Set());
  for (const s of sessions) {
    if (s.status !== "success") continue;
    const hr = new Date(s.startedAt).getHours();
    totals[hr] += s.actualMin;
    daysSeen[hr].add(dayKey(s.startedAt));
  }
  return Array.from({ length: 24 }, (_, h) => ({
    label: `${String(h).padStart(2, "0")}:00`,
    value: daysSeen[h].size ? Math.round(totals[h] / daysSeen[h].size) : 0,
  }));
}

function groveSumMinutes(sessions, start, end) {
  return sessions.reduce((sum, s) => {
    if (s.status !== "success") return sum;
    return s.startedAt >= start.getTime() && s.startedAt < end.getTime() ? sum + s.actualMin : sum;
  }, 0);
}

// This period vs the previous two, each cut off at the same relative
// point ("to this day") so a partial current period is compared fairly.
function groveFocusTrend(anchor, mode, sessions) {
  const now = Date.now();
  const cur = grovePeriodRange(anchor, mode);
  const containsNow = now >= cur.start.getTime() && now < cur.end.getTime();
  const offsetMs = containsNow ? now - cur.start.getTime() : cur.end.getTime() - cur.start.getTime();

  return [0, -1, -2].map((back) => {
    const pAnchor = back === 0 ? anchor : groveShiftPeriod(anchor, mode, back);
    const range = grovePeriodRange(pAnchor, mode);
    const cutoff = new Date(Math.min(range.start.getTime() + offsetMs, range.end.getTime()));
    const minutes = groveSumMinutes(sessions, range.start, cutoff);
    const label = back === 0 ? `This ${groveModeNoun(mode)}` : back === -1 ? `Last ${groveModeNoun(mode)}` : grovePeriodLabel(pAnchor, mode);
    return { label, dateLabel: cutoff.toLocaleDateString(undefined, { month: "numeric", day: "numeric" }), minutes };
  });
}

const GROVE_TAG_COLORS = ["#6fa06a", "#5fa8d3", "#f2b134", "#e07a5f", "#9b5de5", "#4cc9f0", "#f77f00", "#8dc63f"];

function groveTagDistribution(sessions, range, tasksById) {
  const totals = new Map();
  for (const s of sessions) {
    if (s.status !== "success") continue;
    if (s.startedAt < range.start.getTime() || s.startedAt >= range.end.getTime()) continue;
    const label = (s.taskId && tasksById.get(s.taskId)) || "Untagged";
    totals.set(label, (totals.get(label) ?? 0) + s.actualMin);
  }
  ...
  return Array.from(totals.entries())
    ...
    .map(([label, minutes], i) => ({
      ...
      color: GROVE_TAG_COLORS[i % GROVE_TAG_COLORS.length],
    }));
}

// Rounds up to a "nice" axis max (1/2/5 × 10^n) so gridline labels read
// like 40/80/120 instead of an arbitrary number.
function groveNiceMax(value) {
  if (value <= 0) return 4;
...
/* ---- small chart primitives, hand-rolled in plain SVG/CSS so the
   overview doesn't need a charting library ---- */
function GroveBarChart({ data, height = 160, labelEvery = 1 }) {
  const niceMax = groveNiceMax(Math.max(...data.map((d) => d.value), 0));
  const yTicks = [niceMax, Math.round(niceMax * 0.5), 0];
  return (
    <div className="w-full">
      <div className="flex" style={{ height }}>
        <div className="flex w-9 shrink-0 flex-col justify-between pr-2 text-right text-[10px]" style={{ color: "var(--muted-foreground)" }}>
          {yTicks.map((t) => (
            <span key={t}>{t}</span>
          ))}
        </div>
        <div className="relative flex flex-1 items-end gap-[3px]" style={{ borderLeft: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
          {yTicks.map((t) => (
            <div key={t} className="pointer-events-none absolute left-0 right-0 border-t" style={{ borderColor: "var(--border)", opacity: 0.4, bottom: `${(t / niceMax) * 100}%` }} />
          ))}
          {data.map((d, i) => (
            <div key={i} className="relative flex h-full flex-1 items-end justify-center">
              {d.value > 0 ? (
                <div
                  className="w-full max-w-[16px] rounded-t-sm"
                  style={{ height: `${Math.max(2, (d.value / niceMax) * 100)}%`, background: "var(--leaf-deep)" }}
                  title={`${d.label}: ${d.value}m`}
                />
              ) : (
                <div className="mb-0 w-full max-w-[16px] border-t-2 border-dotted" style={{ borderColor: "var(--border)" }} title={`${d.label}: 0m`} />
              )}
            </div>
          ))}
        </div>
      </div>
      <div className="mt-1 flex gap-[3px] pl-9 text-[10px]" style={{ color: "var(--muted-foreground)" }}>
        {data.map((d, i) => (
...
function GroveLineChart({ data, height = 140, labelEvery = 1 }) {
  const niceMax = groveNiceMax(Math.max(...data.map((d) => d.value), 0));
  const yTicks = [niceMax, Math.round(niceMax * 0.5), 0];
  const w = 300;
  const h = 100;
  const stepX = data.length > 1 ? w / (data.length - 1) : 0;
  const points = data.map((d, i) => ({ x: i * stepX, y: h - 6 - (d.value / niceMax) * (h - 16), ...d }));
  ...
  return (
    <div className="w-full">
      <div className="flex" style={{ height }}>
        ...
        <div className="relative flex-1" style={{ borderLeft: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
          <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" width="100%" height="100%">
            <path d={path} fill="none" stroke="var(--leaf-deep)" strokeWidth={2} />
            {peak.value > 0 && (
              <>
                <line x1={peak.x} y1={0} x2={peak.x} y2={h} stroke="var(--border)" strokeDasharray="3 3" />
                <circle cx={peak.x} cy={peak.y} r={3.5} fill="var(--card)" stroke="var(--leaf-deep)" strokeWidth={2} />
              </>
            )}
          </svg>
        </div>
      </div>
      <div className="mt-1 flex pl-9 text-[10px]" style={{ color: "var(--muted-foreground)" }}>
        {data.map((d, i) => (
          <div key={i} className="flex-1 text-center">
            {i % labelEvery === 0 || i === data.length - 1 ? d.label : ""}
          </div>
...
function GroveDonutChart({ segments, size = 170, thickness = 26 }) {
  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:flex-row">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="shrink-0">
        ...
        <text x="50%" y="48%" textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 20, fontWeight: 700, fill: "var(--foreground)" }}>
          {total > 0 ? `${total}m` : "—"}
        </text>
        <text x="50%" y="64%" textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 10, fill: "var(--muted-foreground)" }}>
          focused
        </text>
      </svg>
      <div className="w-full flex-1 space-y-2">
        {segments.length === 0 && <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>No tagged focus time yet in this period.</p>}
        {segments.map((s, i) => (
...
// The "forest bed" — every finished session in the selected period grows
// one real plant here (using the exact plant that session earned), so
// the total focused time literally reads as a small forest.
function GroveForestTile({ items }) {
  const capped = items.slice(0, 48);
  const extra = items.length - capped.length;
  return (
    <div
      className="relative overflow-hidden rounded-2xl"
      style={{ background: "linear-gradient(160deg, #2f6b3f 0%, #1c4327 100%)", border: "1px solid color-mix(in oklab, var(--leaf) 35%, transparent)" }}
    >
      {capped.length === 0 ? (
        <p className="px-6 py-14 text-center text-sm" style={{ color: "rgba(220,245,225,0.75)" }}>
          No sessions finished in this period yet — start a focus session to grow your first tree.
        </p>
      ) : (
        <div className="flex flex-wrap items-end justify-center gap-1 px-4 py-6">
          {capped.map((s, i) => (
            <div key={s.id} className="fg-plant-enter" style={{ animationDelay: `${Math.min(i, 30) * 12}ms` }} title={`${s.actualMin} min · ${s.status}`}>
              <PlantSvg id={s.plantId} progress={s.status === "success" ? 1 : 0.5} withered={s.status !== "success"} size={54} />
            </div>
          ))}
          {extra > 0 && (
            <div className="flex h-[54px] w-[54px] items-center justify-center text-sm font-semibold" style={{ color: "rgba(220,245,225,0.85)" }}>
              +{extra}
            </div>
          )}
        </div>
...
function GroveForestPage({ ctx }) {
  const sessions = useFocusStore((s) => s.sessions);
  const loggdTasks = ctx?.tasks ?? [];
  const tasksById = useMemo(() => new Map(loggdTasks.map((t) => [t.id, t.title])), [loggdTasks]);

  const [mode, setMode] = useState("month");
  const [anchor, setAnchor] = useState(() => new Date());

  const range = useMemo(() => grovePeriodRange(anchor, mode), [anchor, mode]);
  const periodSessions = useMemo(
    () => sessions.filter((s) => s.startedAt >= range.start.getTime() && s.startedAt < range.end.getTime()),
    [sessions, range]
  );
  const periodSuccess = useMemo(() => periodSessions.filter((s) => s.status === "success"), [periodSessions]);
  const periodOther = useMemo(() => periodSessions.filter((s) => s.status !== "success"), [periodSessions]);
  const totalMin = useMemo(() => periodSuccess.reduce((sum, s) => sum + s.actualMin, 0), [periodSuccess]);
  const totalExceededMin = useMemo(() => periodSessions.reduce((sum, s) => sum + (s.exceededMin || 0), 0), [periodSessions]);
  const forestItems = useMemo(() => [...periodSessions].sort((a, b) => a.startedAt - b.startedAt), [periodSessions]);

  const distributionData = useMemo(() => groveBucketsForPeriod(anchor, mode, sessions), [anchor, mode, sessions]);
  const distLabelEvery = Math.max(1, Math.ceil(distributionData.length / 6));
  const weekdayData = useMemo(() => groveWeekdayPattern(sessions), [sessions]);
  const hourData = useMemo(() => groveHourPattern(sessions), [sessions]);
  const trend = useMemo(() => groveFocusTrend(anchor, mode, sessions), [anchor, mode, sessions]);
  const tagData = useMemo(() => groveTagDistribution(sessions, range, tasksById), [sessions, range, tasksById]);

  const bestWeekday = weekdayData.reduce((best, d) => (d.value > best.value ? d : best), weekdayData[0]);
  const bestHour = hourData.reduce((best, d) => (d.value > best.value ? d : best), hourData[0]);
  const trendDelta = trend[0].minutes - trend[1].minutes;
  const trendMax = Math.max(1, ...trend.map((p) => p.minutes));

  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="mb-6 font-display text-2xl font-semibold">Your forest</h1>

      <div className="mb-4 grid grid-cols-4 gap-1 rounded-xl p-1" style={{ background: "var(--secondary)" }}>
        {GROVE_PERIOD_TABS.map((t) => (
          <button
            key={t.id}
            type="button"
            onClick={() => setMode(t.id)}
            className="rounded-lg py-2 text-sm font-semibold transition-colors"
            style={mode === t.id ? { background: "var(--card)", color: "var(--foreground)" } : { color: "var(--muted-foreground)" }}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className="mb-4 flex items-center justify-between">
        <GroveButton size="icon" variant="outline" onClick={() => setAnchor((a) => groveShiftPeriod(a, mode, -1))}>
          <ChevronLeft className="h-4 w-4" />
        </GroveButton>
        <span className="text-sm font-medium">{grovePeriodLabel(anchor, mode)}</span>
        <GroveButton size="icon" variant="outline" onClick={() => setAnchor((a) => groveShiftPeriod(a, mode, 1))}>
          <ChevronRight className="h-4 w-4" />
        </GroveButton>
      </div>

      <GroveForestTile items={forestItems} />
      <div className="mb-8 mt-3 flex items-center justify-center gap-6 text-sm font-medium">
        <span className="flex items-center gap-1.5" style={{ color: "var(--leaf-deep)" }} title="Sessions completed">
          <Sprout className="h-4 w-4" /> {periodSuccess.length}
        </span>
        <span className="flex items-center gap-1.5" style={{ color: "var(--muted-foreground)" }} title="Sessions not completed">
          <Sprout className="h-4 w-4 opacity-50" /> {periodOther.length}
        </span>
      </div>

      <GroveCard className="mb-4">
        <GroveCardContent className="py-5">
          <h2 className="mb-1 font-display text-lg font-semibold">Focused Time Distribution</h2>
          <p className="mb-4 text-sm" style={{ color: "var(--muted-foreground)" }}>
            Total focused time: <span className="font-semibold" style={{ color: "var(--foreground)" }}>{totalMin} mins</span>
            {totalExceededMin > 0 && (
              <>
                {" "}
                · Time exceeded: <span className="font-semibold" style={{ color: "var(--overtime)" }}>+{totalExceededMin} mins</span>
              </>
            )}
          </p>
          <GroveBarChart data={distributionData} labelEvery={distLabelEvery} />
        </GroveCardContent>
      </GroveCard>

      <GroveCard className="mb-4">
        <GroveCardContent className="py-5">
          <h2 className="mb-1 font-display text-lg font-semibold">Most Focused Day of The Week</h2>
          <p className="mb-4 text-sm" style={{ color: "var(--muted-foreground)" }}>
            Most focused on <span className="font-semibold" style={{ color: "var(--leaf-deep)" }}>{bestWeekday?.label ?? "—"}</span> every week in general
          </p>
          <GroveLineChart data={weekdayData} />
        </GroveCardContent>
      </GroveCard>

      <GroveCard className="mb-4">
        <GroveCardContent className="py-5">
          <h2 className="mb-1 font-display text-lg font-semibold">Most Focused Period of The Day</h2>
          <p className="mb-4 text-sm" style={{ color: "var(--muted-foreground)" }}>
            Most focused at <span className="font-semibold" style={{ color: "var(--leaf-deep)" }}>{bestHour?.label ?? "—"}</span> every day in general
          </p>
          <GroveLineChart data={hourData} labelEvery={6} />
        </GroveCardContent>
      </GroveCard>

      <GroveCard className="mb-4">
        <GroveCardContent className="py-5">
          <h2 className="mb-1 font-display text-lg font-semibold">Focus Trend</h2>
          <p className="mb-4 text-sm" style={{ color: "var(--muted-foreground)" }}>
            This {groveModeNoun(mode)} compared with last {groveModeNoun(mode)}:{" "}
            <span className="font-semibold" style={{ color: "var(--leaf-deep)" }}>
              {Math.abs(trendDelta)}m {trendDelta >= 0 ? "longer" : "shorter"}
            </span>
          </p>
          <div className="space-y-3">
            {trend.map((p, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className="w-28 shrink-0 truncate text-xs" style={{ color: "var(--muted-foreground)" }}>
                  {p.label}
                </span>
                <div className="relative h-2.5 flex-1 overflow-hidden rounded-full" style={{ background: "var(--secondary)" }}>
                  <div className="h-full rounded-full transition-all" style={{ width: `${(p.minutes / trendMax) * 100}%`, background: i === 0 ? "var(--leaf-deep)" : "var(--leaf)" }} />
                </div>
                <span className="w-16 shrink-0 text-right text-xs font-medium">{p.minutes}m</span>
              </div>
            ))}
          </div>
        </GroveCardContent>
      ...
      <GroveCard>
        <GroveCardContent className="py-5">
          <h2 className="mb-4 font-display text-lg font-semibold">Tag Distribution</h2>
          <GroveDonutChart segments={tagData} />
        </GroveCardContent>
      </GroveCard>
    </div>
  );
    ...
    <div className="mx-auto max-w-2xl space-y-6 px-4 py-8">
      ...
      <GroveCard>
        ...
        <GroveCardContent className="flex items-center justify-between pb-6">
          <div>
            <p className="text-sm font-medium">Remind me when I switch away</p>
            <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>Just a gentle nudge — your session keeps running and won't break if you switch tabs or browse elsewhere in the app.</p>
          </div>
...
function FocusSessionView({ ctx }) {
  ...
  return (
    <div className="focus-grove-theme min-h-full" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      ...
      {groveTab === "forest" && <GroveForestPage ctx={groveCtx} />}
...
function GoalsView() {
  ...
  return (
    <div className="flex h-full w-full flex-col px-6 py-4">
      <div className="mb-4 flex-shrink-0" style={{ width: "80%", maxWidth: "80%", margin: "0 auto" }}>
...
function HabitScorecard({ ctx }) {
  ...
  return (
    <div className="hs-root" style={{ padding: "32px 16px 80px" }}>
      ...
      <div style={{ width: "80%", maxWidth: "80%", margin: "0 auto", position: "relative" }}>
        {/* Page header */}
...
function HabitChain({
  ...
  // Position among siblings sharing this habit's stack level, so the
  // move-up/move-down controls know when to disable at either end.
  const siblingGroup = siblingsOf(parentId);
  const siblingIndex = siblingGroup.findIndex((h) => h.id === habit.id);
